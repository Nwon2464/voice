"""Launch and consume the native Windows bridge from a WSL process."""
...
from windows_port.bridge_protocol import AUDIO, F8, STATUS, read_frame


class WindowsBridgeClient:
    def __init__(self, on_pcm, on_f8, on_status, on_error):
        self.on_pcm = on_pcm
        self.on_f8 = on_f8
    ...
    def _read(self):
        try:
            while not self.stopped.is_set():
                kind, payload = read_frame(self.process.stdout)
                if kind == AUDIO:
                    self.on_pcm(payload)
                elif kind == F8:
                    self.on_f8()
            ...
            if not self.closing.is_set():
                ...
                self.on_error(
                    RuntimeError(detail or f"Windows bridge exited with status {code}")
                )
    ...
    def _windows_python_path(cls):
        ...
        if not candidates:
            raise RuntimeError("Could not resolve the Windows bridge Python path")
        windows_path = (
            candidates[-1]
            + "\\InterviewAssistantBridge\\.venv\\Scripts\\python.exe"
        )
